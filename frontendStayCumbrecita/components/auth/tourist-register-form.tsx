...
import { AtSign, User, Phone, Lock, Loader2, Home } from "lucide-react"
...
type TouristRegisterFormValues = z.infer<typeof touristRegisterSchema>

interface TouristRegisterFormProps {
  onSubmit: (data: TouristRegisterFormValues) => Promise<void>
}

export default function TouristRegisterForm({ onSubmit }: TouristRegisterFormProps) {
  const [isLoading, setIsLoading] = useState(false)

  const form = useForm<TouristRegisterFormValues>({
  ...
  const handleSubmit = async (data: TouristRegisterFormValues) => {
    try {
      setIsLoading(true)
      await onSubmit(data)
    } finally {
